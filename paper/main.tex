...
% TMDL Theoretical Foundations Diagram
\begin{figure}[htbp]
\centering
\begin{tikzpicture}[
    % Styles
    ...
    adl/.style={pillar, fill=blue!12, draw=blue!50, line width=0.8pt},
    psych/.style={pillar, fill=green!12, draw=green!50!black, line width=0.8pt},
    hai/.style={pillar, fill=orange!12, draw=orange!60, line width=0.8pt},
    result/.style={
        rectangle,
        rounded corners=8pt,
        minimum width=3.5cm,
...
% Three pillars
\node[adl] (adl) at (0,0) {
    \textbf{\footnotesize ADL Framework}\\[2pt]
    Structured language\\
    Context Engineering\\
    Commands \& Workflows
};

\node[psych] (psych) at (3.8,0) {
    \textbf{\footnotesize Organizational}\\
    \textbf{\footnotesize Psychology}\\[2pt]
    Group dynamics\\
    Role taxonomies\\
    IMOI models
};

\node[hai] (hai) at (7.6,0) {
    \textbf{\footnotesize Human-AI}\\
    \textbf{\footnotesize Teaming}\\[2pt]
...
% Result node
\node[result] (tmdl) at (3.8,-3.2) {TMDL};

% Arrows
\draw[arrow] (adl.south) -- ++(0,-0.5) -| ([xshift=-0.5cm]tmdl.north);
\draw[arrow] (psych.south) -- (tmdl.north);
\draw[arrow] (hai.south) -- ++(0,-0.5) -| ([xshift=0.5cm]tmdl.north);

% Labels for foundations
\node[label, above=0.15cm of adl] {Technical Foundation};
\node[label, above=0.15cm of psych] {Psycho-pedagogical Foundation};
\node[label, above=0.15cm of hai] {Collaborative Foundation};
...
TMDL positions itself firmly within the ``teammate'' paradigm, but with important qualifications:
...
A TMDL document consists of five main sections plus metadata:
...
\hline
\textbf{KNOWLEDGE} & External documents (domain \& context) \\
\hline
\end{tabular}
}
