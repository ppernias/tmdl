...
% TMDL Structure Diagram - 2x2 layout
\begin{figure}[htbp]
\centering
...
% METADATA section
\node[metadata] (meta) at (2.8, 5.2) {
    \textbf{\scriptsize METADATA}\\
    id, name, version, author, tags
};

% Static definition group - 2x2 grid
\node[identity] (identity) at (1.2, 3.2) {
    \textbf{\scriptsize IDENTITY}\\
    personality\\
    tone, quirks
};

\node[role] (role) at (4.4, 3.2) {
    \textbf{\scriptsize ROLE}\\
    type, expertise\\
    boundaries
};

\node[collab] (collab) at (1.2, 1.7) {
    \textbf{\scriptsize COLLAB.}\\
    contribution\\
    protocols
};

\node[tools] (tools) at (4.4, 1.7) {
...
\begin{scope}[on background layer]
    \node[groupbox, fill=blue!5, fit=(identity)(role)(collab)(tools), inner sep=6pt] (staticbox) {};
\end{scope}
% Label ABOVE the static box
\node[grouplabel, anchor=south] at (staticbox.north) {Static Definition};

% Knowledge section (dynamic)
\node[knowledge] (know) at (2.8, -0.3) {
...
\begin{scope}[on background layer]
    \node[groupbox, fill=yellow!5, fit=(know), inner sep=6pt] (dynbox) {};
\end{scope}
% Label ABOVE the dynamic box
\node[grouplabel, anchor=south] at (dynbox.north) {Dynamic Context};
